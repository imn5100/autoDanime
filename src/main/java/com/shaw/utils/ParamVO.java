...
public class ParamVO {
	...
	// private static final SimpleDateFormat format_date = new
	...
	public static final String XML_FORMAT_ERR = "907";
	public static final String XML_FORMAT_ERR_DESC = "XML格式错误";

	private Element xmlroot;

	public ParamVO(Element element) {
		xmlroot = element;
	}

	public ParamVO(String base) {
	...
	public void addStringValue(String path, String value) {
		...
		if (element == null) {
			throw new RuntimeException(
					XML_FORMAT_ERR + SPLIT + (new StringBuilder("指定路径")).append(path).append("的节点创建失败").toString());
		}
		if (paths[1] != null) {
	...
	public String getStringValue(String path) {
		...
		if (element == null) {
			throw new RuntimeException(
					XML_FORMAT_ERR + SPLIT + (new StringBuilder("指定路径")).append(path).append("的节点未找到").toString());
		}
		if (paths[1] != null) {
			...
			if (attr != null) {
				...
				throw new RuntimeException(
						XML_FORMAT_ERR + SPLIT + (new StringBuilder("当前路径")).append(path).append("的属性未定义").toString());
			}
		} else {
	...
	public Hashtable<String, String> getXmlAttrs(String path) {
		String paths[][] = decodePathString(path);
		if (paths[1] != null) {
			throw new RuntimeException(
					XML_FORMAT_ERR + SPLIT + (new StringBuilder("当前路径")).append(path).append("为属性路径").toString());
		}
		String dirs[] = paths[0];
	...
	public ParamVO getXmlNode(String path) {
		...
		if (element == xmlroot) {
			...
			return new ParamVO(element);
		}
	}

	public List<ParamVO> getXmlNodeList(String path) {
		String paths[][] = decodePathString(path);
		if (paths[1] != null) {
			throw new RuntimeException(
					XML_FORMAT_ERR + SPLIT + (new StringBuilder("当前路径")).append(path).append("为属性路径").toString());
		...
		for (int idx = 0; idx < elmlist.size(); idx++) {
			volist.add(new ParamVO((Element) elmlist.get(idx)));
		}
		return volist;
	}
	...
	public ParamVO addXmlNode(String path) {
		...
		if (element == null) {
			throw new RuntimeException(
					XML_FORMAT_ERR + SPLIT + (new StringBuilder("指定路径")).append(path).append("的节点未创建成功").toString());
		}
		if (element == xmlroot) {
			return this;
		} else {
		...
		for (int idx = 0; idx < pathlen; idx++) {
			...
			if (itemidx < 0) {
				if (idx == pathlen - 1) {
					...
					throw new RuntimeException(XML_FORMAT_ERR + SPLIT
							+ (new StringBuilder("元素节点")).append(pathdir[idx]).append("不明确").toString());
				}
			} else {
				if (subnodes.size() == 0) {
					throw new RuntimeException(XML_FORMAT_ERR + SPLIT
							+ (new StringBuilder("元素节点")).append(itemname).append("未定义").toString());
				}
				if (itemidx >= subnodes.size()) {
					throw new RuntimeException(XML_FORMAT_ERR + SPLIT
							+ (new StringBuilder("元素节点")).append(pathdir[idx]).append("未定义").toString());
				}
	...
	private String[][] decodePathString(String path) {
		...
		for (int len = temp.length; idx < len; idx++) {
			if (idx < len - 1 && temp[idx].indexOf("@") != -1) {
				throw new RuntimeException(XML_FORMAT_ERR + SPLIT
						+ (new StringBuilder("")).append(path).append(":中间路径不能够有@符号").toString());
			}
			if (idx == len - 1 && split(temp[idx], "@").length > 2) {
				throw new RuntimeException(XML_FORMAT_ERR + SPLIT
						+ (new StringBuilder("")).append(path).append(":最后属性项有且只能有一个@符号").toString());
			}
			if (idx == len - 1 && temp[idx].indexOf("@") != -1 && !temp[idx].startsWith("@")) {
				throw new RuntimeException(XML_FORMAT_ERR + SPLIT
						+ (new StringBuilder("")).append(path).append(":最后属性项只能以@开头").toString());
			}
			if (split(temp[idx], "[").length > 2 || split(temp[idx], "]").length > 2) {
				throw new RuntimeException(XML_FORMAT_ERR + SPLIT
						+ (new StringBuilder("")).append(path).append(":路径项的子项标志[]有且只能有一对").toString());
			}
			int spos = temp[idx].indexOf("[");
			int epos = temp[idx].indexOf("]");
			if (spos * epos < 0) {
				throw new RuntimeException(XML_FORMAT_ERR + SPLIT
						+ (new StringBuilder("")).append(path).append(":路径项的子项标志[]不匹配").toString());
			}
			if (epos != -1 && epos != temp[idx].length() - 1) {
				throw new RuntimeException(XML_FORMAT_ERR + SPLIT
						+ (new StringBuilder("")).append(path).append(":路径项的子项标志[]必须以]结尾").toString());
			}
		}
