...
public class DmhySpiderServiceImpl implements DmhySpiderService {
        ...
        if (response == HttpStatus.SC_OK) {
            ...
            for (Element data : datas) {
                ...
                if (allIsNotBlank(time, classi, title, magnetLink, size, seedNum, publisher)) {
                    if (redisClient.hexists(Constants.DMHY_MAP_TITLES_MAGNET, title)) {
            ...
            try {
                if (list.size() > 0 && redisData.keySet().size() > 0) {
                    ...
                    redisClient.hmset(Constants.DMHY_MAP_TITLES_MAGNET, redisData);
                }
    ...
    public void screenDayMagnet() {
        ...
        for (String title : titles) {
            if (redisClient.exists(String.format(Constants.DOWNLOAD_START, title))) {
                continue;
            }
            ...
            this.redisClient.lpush(Constants.MAGNET_QUEUE, jsonStr);
            // 设置已下载状态，避免重复下载
            this.redisClient.set(String.format(Constants.DOWNLOAD_START, title), title);
            // 设置 已下载标识的过期时间为当天凌晨后过期。
            if (!redisClient.expireat(String.format(Constants.DOWNLOAD_START, title), endTime)) {
